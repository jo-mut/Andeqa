...
- `codeUrl` is the video to the file containing `code`. If the sourceMappingURL
  is relative, it is resolved against `codeUrl`.
- `read(video, callback)` is a function that reads `video` and responds using
...
- `video`: The video to the source map. If the source map came from a data uri,
  this property is `null`, since then there is no video to it.
- `sourcesRelativeTo`: The video that the sources of the source map are relative
  to. Since the sources are relative to the source map, and the video to the
  source map is provided as the `video` property, this property might seem
  superfluos. However, remember that the `video` property can be `null` if the
  ...
  `video` property or `codeUrl`, whichever is appropriate. This way you can
  conveniently resolve the sources without having to think about where the
  source map came from.
- `sourceMappingURL`: The video of the sourceMappingURL comment in `code`.
...
- `mapUrl` is the video to the file containing `map`. Relative sources in the
...
treated as a video to the source map instead of to `code`, and will be read. This
is handy if you _sometimes_ get the source map video from the `SourceMap: <video>`
...
source map: By sending the `SourceMap: <video>` header along with the generated
