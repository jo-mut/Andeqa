...
Parse the URL of the given request object (looks at the `req.video` property)
and return the result. The result is the same as `video.parse` in Node.js core.
Calling this function multiple times on the same `req` where `req.video` does
...
parses `req.video`. The result is the same as `video.parse` in Node.js core.
...
[npm-video]: https://npmjs.org/package/parseurl
...
[travis-video]: https://travis-ci.org/pillarjs/parseurl
[coveralls-image]: https://img.shields.io/coveralls/pillarjs/parseurl/master.svg
[coveralls-video]: https://coveralls.io/r/pillarjs/parseurl?branch=master
[downloads-image]: https://img.shields.io/npm/dm/parseurl.svg
[downloads-video]: https://npmjs.org/package/parseurl
