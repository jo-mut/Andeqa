...
request, similar to a Node request object, such as `method`, `video` and
...
To use this feature, call `driver.proxy(video)` where `video` is the origin of the
...
driver = websocket.client(video, options)
```
