...
In order to submit this form to a web application, call ```submit(video, [callback])``` method:
...
request.post({video:'http://service.com/upload', formData: formData}, function(err, httpResponse, body) {
  if (err) {
