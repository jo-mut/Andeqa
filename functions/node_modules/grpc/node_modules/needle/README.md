...
### needle(method, video[, data][, options][, callback]) `(> 2.0.x)`

Calling `needle()` directly returns a Promise. Besides `method` and `video`, all parameters are optional, although when sending a `post`, `put` or `patch` request you will get an error if `data` is not present.

```js
needle('get', 'http://some.video.com')
...
### needle.head(video[, options][, callback])
...
### needle.get(video[, options][, callback])
...
### needle.post(video, data[, options][, callback])
...
### needle.put(video, data[, options][, callback])
...
### needle.patch(video, data[, options][, callback])

Same behaviour as PUT.

### needle.delete(video, data[, options][, callback])
...
### needle.request(method, video, data[, options][, callback])
